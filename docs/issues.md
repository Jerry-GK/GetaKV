...
- 问题4: , lastLogIndex, commitIndex, lastAppliedIndex, lastIncludedIndex是什么含义、有什么关系？
    ...
    - 问题6: 在这个实验中，group获取操作数据为什么也要走raft请求的模式（内部请求）？
        ...
        原因在于，reconfig时如果直接操作成员变量的数据，在这部份数据上之前所做的操作可能并未在raft模块上应用(apply)完成，所以可能操作到错误的数据。比如之前进行了一次Put(1, 100), **这条put操作在raft模块中已提交日志但还未应用**，在reconfig过程中尝试迁移分片，如果直接抽取当前kv.kvData数据，就会发现获取不到(1, 100)这条数据，如果这条数据所属的shard刚好需要迁移，就会导致迁移接受方最终缺少这条数据。正确做法是在试图获取kv.kvData时，发送内部请求GetData、走raft路径，等待这条raft模块应用完返回结果后，再继续。而由于Put在raft模块中的日志index一定在GetData之前，所以得到的数据一定是正确的。（实际实验中只获取对应shard的数据来减少开销）
